...
let specTags = ['style', 'script', 'template'];
...
module.exports = acorn => {
    ...
    (0, (_assign || _load_assign()).default)(acorn.Parser.prototype, {
        ...
        lesx_parseAttributeValue() {
            switch (this.type) {
                case tt.braceL:
                    ...
                    if (node.expression.type === 'LesxEmptyExpression') this.raise(node.start, 'Lesx attributes must only be assigned a non-empty expression');
                    return node;
        ...
        lesx_parseExpressionContainer() {
            var node = this.startNode();
            this.next();
            node.expression = this.type === tt.braceR ? this.lesx_parseEmptyExpression() : this.parseExpression();
        ...
        lesx_parseAttribute() {
            ...
            node.value = this.eat(tt.eq) ? this.lesx_parseAttributeValue() : null;
    ...
    acorn.plugins.lesx = function (instance, opts) {
        ...
        if (Array.isArray(opts.specTags)) {
            specTags = opts.specTags;
        }

        // 设置lesx插件的基础配置
        instance.options.plugins.lesx = (0, (_assign || _load_assign()).default)({}, opts, {
        ...
        instance.extend('parseExprAtom', function (inner) {
            return function (refShortHandDefaultPos) {
                if (this.type === tt.lesxText) {
                    // lesx文本
                    return this.lesx_parseText(this.value);
                } else if (this.type === tt.lesxTagStart) {
                    // lesx元素
        ...
        const extendObj = {
            ...
            readToken(inner) {
                ...
                return function (code) {
                    // code是this.input当前位置(this.pos)的Unicode编码：this.input.charCodeAt(this.pos)
                    var context = this.curContext(); // 获取当前上下文

                    if (context === tc.j_expr) {
                        // 解析到开始标签的 > 符号以及标签之间的文本的时候
            ...
            updateContext(inner) {
                return function (prevType) {
                    ...
                    if (this.type == tt.braceL) {
                        ...
                        this.exprAllowed = true;
                    } else if (this.type === tt.slash && prevType === tt.lesxTagStart) {
                        // 结束标签
        ...
        (0, (_keys || _load_keys()).default)(extendObj).forEach(key => {
